...
After the pack is activated, add the collection rule from the file `resources/collection_group/network.json`.<br><a href="https://github.com/cloudfabrix/rda_packs/tree/main/Network%20Suite/2.0.0/resources/collection_group" target="_blank">Click here for collection group files</a><br>
...
- Copy the required Telegraf config files from `resources/telegraf_config_files/` to path: `/opt/rdaf/event_gateway/telegraf/config/conf.d`<br><a href="https://github.com/cloudfabrix/rda_packs/tree/main/Network%20Suite/2.0.0/resources/telegraf_config_files" target="_blank">Click here for Telegraf config files</a><br>
...
| 3    | Complete the Manual Steps (Post Activation) listed above — add the collection rule and configure the Telegraf plugins (SNMP, ICMP, SSL, Net Response, Syslog UDP, SNMP Trap). |  
