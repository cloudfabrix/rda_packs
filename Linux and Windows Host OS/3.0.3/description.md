...
| 5    | Under `Configure and Manage` -> `Run Discovery` use `Run Access Verification` option to perform the credential check for hosts. The verification results can be seen in `Access Verification Status` of `Status` dashboard page.|  
