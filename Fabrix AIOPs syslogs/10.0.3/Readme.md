...
| 3    | Update `syslog_alert_rules_dict` dataset. Use `Configure and Manage`-->`Syslog Management`-->`Configure Artifacts`-->`Row Level Actions`-->`import` action. Upload the csv file. | 
| 4    | Update `syslog_rules_dict` (syslog parser rules). Use `Configure and Manage`-->`Syslog Management`-->`Syslog Parsers`-->`Syslog processing rules`-->`Row Level Actions` to add, edit, delete rules, or test Grok. |     
| 5    | Review the blueprint for the scheduled pipeline that raises alerts.|
| 6    | Syslog data is visualized under `Events` -> `Syslogs`. | 
| 7    | Syslog alerts and incidents are visualized under Alerts and Incidents page. |   
